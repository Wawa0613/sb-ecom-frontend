...
export const addToCart = (data, qty = 1, toast) => 
    (dispatch, getState) => {
        ...
        if (isQuantityExist) {
            ...
            // localStorage 更新已在 cartReducer 中处理
        } else {
            // error
...
export const authenticateSignInUser 
    = (sendData, toast, reset, navigate, setLoader) => async (dispatch) => {
        try {
            setLoader(true);
            // 修复：登录请求加 withCredentials: true，确保 cookie 能写入
...
export const addUpdateUserAddress =
    ...
    */
    ...
    try {
        if (!addressId) {
            const { data } = await api.post("/addresses", sendData, { withCredentials: true });
        } else {
            await api.put(`/addresses/${addressId}`, sendData, { withCredentials: true });
        }
...
export const deleteUserAddress = 
    ...
    try {
        dispatch({ type: "BUTTON_LOADER" });
        await api.delete(`/addresses/${addressId}`, { withCredentials: true });
        dispatch({ type: "IS_SUCCESS" });
        dispatch(getUserAddresses());
...
export const getUserAddresses = () => async (dispatch, getState) => {
    try {
        dispatch({ type: "IS_FETCHING" });
        const { data } = await api.get(`/addresses`, { withCredentials: true });
...
export const stripePaymentConfirmation 
    = (sendData, setErrorMesssage, setLoadng, toast) => async (dispatch, getState) => {
        try {
            const response  = await api.post("/order/users/payments/online", sendData, { withCredentials: true });
            if (response.data) {
